...
try:
                ...
                if len(e2.get())!=0:
                    if (len(t2.get("1.0", "end-1c"))==0):
                        ...
                        if temp:
                            ...
                            e2.delete(0,'end')
                            t2.delete("1.0",END)
                            messagebox.showinfo("Congratulations!!","Note added succeessfully")
                            
                        else:
                            sol = messagebox.askyesno("QUESTION!","Do you want to add again?")
                            if sol:
                                add_new_notes()
                        ...
                        e2.delete(0,'end')
                        t2.delete("1.0",END)
                        l4 = Label(top1,text = 'Data inserted successfully')
                        l4.place(x = 100,y = 500)
                else:
                    messagebox.showinfo("EMPTY TITLE!!","You have not given title to your notes.\nNotes without title cannot be added.")
            except Exception as e:
                messagebox.showinfo("DUPLICATE NOTES","Note with name %s already exists.Give new name. "%(e2.get()))
    ...
    def update_notes():
        ...
        def update_to_database():
            ...
            if rows_count>0:
                ...
                l7 = Label(top2,text = 'There is no notes found titled %s'%(e3.get()),width = 50)
                l7.place(x = 150,y = 480)

            t3.delete("1.0",END)
            e3.delete(0,'end')

        b8 = Button(top2,text = 'Save New Changes',bg = 'green',width = 20,command = update_to_database)
    ...
    def delete_notes():
        ...
        def delete_from_database():
            result = messagebox.askokcancel("Delete Notes","Are you sure you want to delete?")
            if result:
        ...
        def deleteall_from_database():
            result = messagebox.askokcancel("Delete All Notes","Are you sure you want to delete all notes ?")
            if result:
                rows_count = cursor.execute(""" DELETE from note_making """)
                con.commit()
                if rows_count>0:
                    messagebox.showinfo("Success","All Notes are deleted succeessfully")
                else:
                    messagebox.showinfo("Failure","There are no notes present.")
    ...
    b6 = Button(root,text = 'Update Notes>>',height = 2,width = 30,bg ='red',foreground='white',font=('times', 10, 'bold'),command =update_notes)
    b6.place(x = 20,y = 70)
    ...
    t4 = Text(root,height = 2,width = 70,bg = 'pink',state=DISABLED)
    t4.place(x = 0,y =280)
    ...
    t1 = Text(root,wrap = NONE,height = 20,width = 70,bg = 'pink',state=DISABLED,yscrollcommand = s1.set,xscrollcommand = s5.set)
